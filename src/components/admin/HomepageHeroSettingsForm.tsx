...
export default function HomepageHeroSettingsForm({ initial }: Props) {
  ...
  return (
    <form className="admin-form" onSubmit={handleSubmit}>
      <fieldset className="admin-catalog-fieldset">
        ...
        <div className="admin-form-group">
          ...
          <p className="admin-field-hint">
            Có thể dùng đường dẫn nội bộ như /lien-he hoặc anchor như #home-categories.
          </p>
        </div>
      ...
      {message && (
        <p className={message.type === "success" ? "admin-success" : "admin-error"}>
          {message.text}
        </p>
      )}

      <button type="submit" className="admin-btn admin-btn--primary" disabled={loading}>
        {loading ? "Đang lưu…" : "Lưu nội dung hero"}
      </button>
    </form>
  );
}
