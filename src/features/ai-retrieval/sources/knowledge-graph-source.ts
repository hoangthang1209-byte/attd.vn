 ...
 *
 * Feature flag: KNOWLEDGE_GRAPH_EXPANSION_ENABLED (default false).
 * When disabled: no graph queries, no retrieval behavior changes.
 *
 * Graph nodes enrich scope only. Authoritative facts still come from
 * product / knowledge / media / seo adapters — never from graph metadata.
 */
...
export async function expandRetrievalScopeViaKnowledgeGraph(
  ...
  // Explicit entity scope or strong product match required
  ...
  // Lazy import so disabled path never loads prisma graph queries in tests that mock env off
...
export function attachGraphProvenanceToFacts(
  ...
  const next = facts.map((fact) => {
    ...
    return {
      ...
      // Bonus must not outrank direct authoritative fact matching
