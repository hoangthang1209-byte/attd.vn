...
describe("P0.2A acceptance hardening", () => {
  ...
  it("fast-create wizard does not expose legacy variant price fields", () => {
    const wizard = readRepoFile("src/components/admin/products/ProductFastCreateWizard.tsx");
    assert.equal(wizard.includes("wholesalePrice"), false);
    assert.equal(wizard.includes("dealerPrice"), false);
    assert.ok(wizard.includes("PRICING_CLARITY_HELPER"));
  });
