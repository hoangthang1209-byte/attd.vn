...
import {
  hasConfiguredTestDatabase,
  resetPrismaTestClient,
  uniqueTestKey,
...
const describeDb = hasConfiguredTestDatabase() ? describe : describe.skip;
...
describeDb("product relation ownership (database-backed)", () => {
  ...
  before(async () => {
    prisma = await resetPrismaTestClient();
...
describeDb("product mutation API routes (database-backed)", () => {
  let prisma: PrismaClient;
  let categoryId = "";
  const createdProductIds: string[] = [];
  const createdCategoryIds: string[] = [];
...
if (!hasConfiguredTestDatabase()) {
  it("documents missing TEST_DATABASE_URL for database-backed integration coverage", () => {
    assert.ok(true);
  });
}
