...
import {
  buildProductionSheetDocumentUrl,
  resolveOrderDocumentBaseUrl,
} from "@/features/orders/production-sheet/production-sheet-pdf-url";
import {
  createOrderPdfTraceId,
  OrderPdfChromiumError,
} from "@/features/orders/pdf/order-pdf-chromium-error";

export { resolveOrderDocumentBaseUrl, buildProductionSheetDocumentUrl };
...
const DOCUMENT_FETCH_USER_AGENT = "ATTD Production Sheet PDF Renderer";

function logStage(traceId: string, stage: string, extra?: Record<string, unknown>) {
  console.log("[production-sheet-pdf]", { traceId, stage, ...extra });
}
...
async function closeBrowserSafely(traceId: string, browser: Browser | null) {
  ...
  try {
    ...
    console.error("[production-sheet-pdf] browser close failed", { traceId, closeError });
  }
}

export async function generateProductionSheetHtmlPdf(
  documentUrl: string,
  meta?: { orderId?: string },
): Promise<Buffer> {
  ...
  logStage(traceId, "start", { documentUrl, orderId });
  ...
  try {
    ...
    await page.waitForSelector(".production-sheet-document", { timeout: 30_000 });
    await page.waitForSelector('[data-production-sheet-pdf-ready="true"]', { timeout: 30_000 });
    await Promise.race([
      page.evaluate(async () => {
    ...
    logStage(traceId, "complete", { orderId, bytes: buffer.length });
    return buffer;
  } catch (error) {
    console.error("[production-sheet-pdf] chromium failed", {
      traceId,
      orderId,
      documentUrl,
      error,
...
export async function generateProductionSheetPdf(options: HtmlPdfOptions): Promise<Buffer> {
  const baseUrl = resolveOrderDocumentBaseUrl(options.requestHeaders);
  const documentUrl = buildProductionSheetDocumentUrl(options.orderId, {
    mode: "pdf",
    baseUrl,
  ...
  return generateProductionSheetHtmlPdf(documentUrl, { orderId: options.orderId });
}
