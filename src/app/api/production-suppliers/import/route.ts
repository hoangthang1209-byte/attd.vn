...
import {
  importProductionSuppliersCsv,
  previewProductionSuppliersCsv,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === "preview") {
      const preview = await previewProductionSuppliersCsv(buffer);
      return NextResponse.json(preview);
    }
    const summary = await importProductionSuppliersCsv(buffer);
...
export async function GET() {
  const csv = `${PRODUCTION_MASTER_CSV_TEMPLATES.supplier}\n`;
  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="production-suppliers-template.csv"',
    },
  });
}
