...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    const status = parseExecutionStatusBody(body as Record<string, unknown>);
    const execution = await updateDeliveryExecutionStatus(id, executionId, status);
    ...
    return NextResponse.json({ message: "Không thể cập nhật trạng thái chuyến" }, { status: 500 });
  }
}
