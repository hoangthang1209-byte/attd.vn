...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    const summary = await reopenOrderActualCost(id, session.userId);
    return NextResponse.json({ actualCost: summary });
  } catch (err) {
    if (err instanceof OrderActualCostValidationError) {
      return NextResponse.json({ message: err.message }, { status: 400 });
    }
    console.error("[POST /api/orders/[id]/actual-cost/reopen]", err);
    return NextResponse.json({ message: "Không thể mở lại chi phí thực tế" }, { status: 500 });
  }
}
