...
import {
  importProductionMaterialsCsv,
  previewProductionMaterialsCsv,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === "preview") {
      const preview = await previewProductionMaterialsCsv(buffer);
      return NextResponse.json(preview);
    }
    const summary = await importProductionMaterialsCsv(buffer);
...
export async function GET() {
  const csv = `${PRODUCTION_MASTER_CSV_TEMPLATES.material}\n`;
  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="production-materials-template.csv"',
    },
  });
}
