...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await listProductionMaterials({
    ...
    return NextResponse.json({ message: "Không thể tải danh sách vật liệu." }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const created = await createProductionMaterial({
    ...
    return NextResponse.json({ message: "Không thể tạo vật liệu." }, { status: 500 });
  }
}
