...
import { cancelContentProposal } from "@/features/content-generation/services/proposal.wiring";
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    const run = await cancelContentProposal(id);
    return NextResponse.json({ proposal: toSafeProposalDetail(run), message: "Đã huỷ đề xuất." });
  } catch (err) {
