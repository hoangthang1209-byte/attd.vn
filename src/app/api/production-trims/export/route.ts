...
import { exportProductionTrimsCsv } from "@/features/production-master/production-master-export.service";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const csv = await exportProductionTrimsCsv({
      search: searchParams.get("search") ?? undefined,
      activeOnly: searchParams.get("activeOnly") === "true",
      category: searchParams.get("category") ?? undefined,
    });
    return downloadCsvResponse("production-trims", csv);
  } catch {
