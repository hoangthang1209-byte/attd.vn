...
import {
  importProductionTrimsCsv,
  previewProductionTrimsCsv,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === "preview") {
      const preview = await previewProductionTrimsCsv(buffer);
      return NextResponse.json(preview);
    }
    const summary = await importProductionTrimsCsv(buffer);
...
export async function GET() {
  const csv = `${PRODUCTION_MASTER_CSV_TEMPLATES.trim}\n`;
  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="production-trims-template.csv"',
    },
  });
}
