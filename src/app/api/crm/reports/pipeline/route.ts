...
import { getPipelineReport } from "@/features/crm/services/crm-reporting.service";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await getPipelineReport(session, filters);
    return NextResponse.json(data);
  } catch (error) {
    if (error instanceof Error && error.message === "FORBIDDEN") {
      return NextResponse.json({ message: "Bạn không có quyền xem báo cáo CRM." }, { status: 403 });
    }
    return NextResponse.json({ message: "Không thể tải báo cáo pipeline." }, { status: 500 });
  }
}
